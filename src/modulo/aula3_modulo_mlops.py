...
def residual_calculator(seismic_GT = None, seismic_original = None):

    seismic_slice_residual = seismic_GT[:,-1] - seismic_original[:,-1]

    seismic_slice_residual_reshape = seismic_slice_residual.reshape(len(seismic_slice_residual),1)

    seismic_slice_residual_final = np.hstack([seismic_original[:,:-1], seismic_slice_residual_reshape])
    
    return seismic_slice_residual_final
...
def ML_model_evaluation(dados_simulacao=None, proporcao_treino=None):    
    """
    ...
    return sim_estimado, y, nrms_teste, r2_teste, mape_teste, dict_params, ET, X

def plot_simulation_distribution(sim_array_xyzprop=None, bins=None, title="Distribuição da Propriedade da Simulação", xlabel="Valor da Propriedade",
                                ylabel="Frequência",alpha=0.3):
    """
...
def simulation_nan_treatment(simulation=None, value=0, method='replace'):
    """
